...
test('solveGeometry changes Ca/Cb monotonically with position', () => {
  ...
  assert.ok(
    left.caF > right.caF,
    `left.caF=${left.caF}, right.caF=${right.caF}`,
  );
  assert.ok(
    left.cbF < right.cbF,
    `left.cbF=${left.cbF}, right.cbF=${right.cbF}`,
  );
});

test('solveGeometry clamps distances with min gap at extremes', () => {
  const left = solveGeometry({
    ...DEFAULT_INPUTS,
    position: 0.0,
    minGapMm: 0.05,
  });
  const right = solveGeometry({
    ...DEFAULT_INPUTS,
    position: 1.0,
    minGapMm: 0.05,
  });
...
test('solveGeometry throws RangeError for invalid ranges', () => {
  ...
  assert.throws(
    () =>
      solveGeometry({
        ...DEFAULT_INPUTS,
        minGapMm: DEFAULT_INPUTS.totalGapMm + 0.1,
      }),
    RangeError,
